...
const getBgVariantStyle = (variant: ButtonProps["bgVariant"]) =>{
    switch (variant) {
        ...
        case "outline": return "bg-transperent border-neutral-300 border-[0.5px]"
...
const CustomButton = ({
    ...
    >
        {IconLeft && <IconLeft/>}
        <Text className={`text-lg font-bold ${getTextVariantStyle(textVariant)} ${className}`}>{title}</Text>
        {IconRight && <IconRight/>}
    </TouchableOpacity>
)
